            ...
            for alignment in entry.alignments:
                for hsp in alignment.hsps:
                    ...
                    if ident > min_ident and coverage:
                        ...
                        # split hit def
                        hit_def_names = alignment.hit_def.split(';')
                        for hit_def_name in hit_def_names:
                            hit_def_name = hit_def_name.strip()
                            if hit_def_name.startswith('RecName: Full='):
                                accession = hit_def_name[14:].strip()
                                break
